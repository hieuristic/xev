...
Mesh Scene::parse_mesh(const tinygltf::Model &model,
                       const tinygltf::Node &node, glm::mat4 model_mat) {
  ...
  return Mesh(std::move(name), model_mat, std::move(primitives),
...
void Scene::load_gltf(std::string_view filepath) {
  ...
  while (!to_visit.empty()) {
    ...
    if (node.camera != -1 && node.name == "cam.active") {
      ...
      m_active_cam = Camera(abs_rot, abs_pos, glm::degrees(static_cast<float>(camera.perspective.yfov)));
      cam_found = true;
    }

    // parsing geometries
    if (node.mesh != -1) {
      meshes.emplace_back(parse_mesh(model, node, abs_mat));
    }
...
void Scene::destroy(Backend &backend) {
